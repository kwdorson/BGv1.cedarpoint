...
if (getNumber(MissionconfigFile >> (format["Maverick_IntroCam_%1", worldName]) >> "Info" >> "music") == 1) then {
	0 fadeMusic (getNumber(MissionconfigFile >> (format["Maverick_IntroCam_%1", worldName]) >> "Info" >> "musicVolume"));
	playMusic (getText(MissionconfigFile >> (format["Maverick_IntroCam_%1", worldName]) >> "Info" >> "musicName"));
	0 fadeSound 0;
};
