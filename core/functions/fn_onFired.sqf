...
if((playerside isEqualTo civilian) && safeZone) then {
	...
	if(_curGun in [primaryWeapon player,secondaryWeapon player,handgunWeapon player]) then 
	{
		player setAmmo [_curGun, 0];
		[] call life_fnc_saveGear;
		["I shouldn't shoot my weapon in a safezone, the mag has been removed.",false,"slow"] call life_fnc_notificationSystem;
	};
};
