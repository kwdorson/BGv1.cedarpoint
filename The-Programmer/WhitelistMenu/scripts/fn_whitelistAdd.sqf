...
if (_type select [0,8] == "coplevel") exitWith {[] spawn max_whitelist_fnc_copLevelChange;};
if (_type select [0,8] == "medlevel") exitWith {[] spawn max_whitelist_fnc_medLevelChange;};
if (_type select [0,8] == "donlevel") exitWith {[] spawn max_whitelist_fnc_donLevelChange;};
...
hint format [(["STR_SUCCES_ADD_WHITELIST"] call max_whitelist_fnc_localize),localize _displayName,_unit getVariable ["realname",name _unit]];