/*
    ...
    Owner's name:
*/
if !([(name player),(getPlayerUID player)] in maxence_ministres) exitWith {};
_openDialog = param [0,0,[0]];

if (_openDialog isEqualTo 1) then {createDialog "maxence_ministre";};
disableSerialization;

_display = findDisplay 1500;

if (getText (missionConfigFile >> "Max_Settings_Gouvernement" >> "default_lang") isEqualTo "fr") then {
    (_display displayCtrl 1000) ctrlSetText "The-Programmer\Gouvernement\textures\ministre_fond.paa";
} else {
    if (getText (missionConfigFile >> "Max_Settings_Gouvernement" >> "default_lang") isEqualTo "de") then {
        (_display displayCtrl 1000) ctrlSetText "The-Programmer\Gouvernement\textures\ministre_fond_de.paa";
    } else {
        if (getText (missionConfigFile >> "Max_Settings_Gouvernement" >> "default_lang") isEqualTo "es") then {
            (_display displayCtrl 1000) ctrlSetText "The-Programmer\Gouvernement\textures\ministre_fond_es.paa";
        } else {
            (_display displayCtrl 1000) ctrlSetText "The-Programmer\Gouvernement\textures\ministre_fond_en.paa";
        };
    };
};
