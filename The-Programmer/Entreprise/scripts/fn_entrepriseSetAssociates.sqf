...
if (_pid == (getPlayerUID player)) exitWith {hint (["STR_CANT_PROMOTE_YOURSELF"] call max_entreprise_fnc_localize)};
...
if (_rank == 2) exitWith {hint (["STR_CANT_PROMOTE"] call max_entreprise_fnc_localize)};
if (_rank == 1) exitWith {
    _action = [
        format [(["STR_RANK_PDG_TEXT"] call max_entreprise_fnc_localize),_name],
        (["STR_RANK_PDG_HEADER"] call max_entreprise_fnc_localize),
    ...
    if (_index2 == -1) exitWith {};
    if (_rank2 != 2) exitWith {};

    _members set [_index,[_pid,_name,(_rank + 1)]];
    _members set [_index2,[(getPlayerUID player),(name player),(_rank2 - 1)]];
    _entreprise setVariable ["entreprise_members",_members,true];
    _entreprise_id = _entreprise getVariable ["entreprise_id",0];
    [_entreprise_id,1,(_entreprise getVariable ["entreprise_members",[]])] remoteExecCall ["max_entreprise_fnc_updateEntreprise",2];

    _entreprise setVariable ["entreprise_pdg_pid",_pid,true];
    _entreprise setVariable ["entreprise_pdg_name",_name,true];
    [_entreprise_id,11,(_entreprise getVariable ["entreprise_pdg_pid",""]),(_entreprise getVariable ["entreprise_pdg_name",""])] remoteExecCall ["max_entreprise_fnc_updateEntreprise",2];
    [_entreprise_id,(getPlayerUID player),_pid] remoteExecCall ["max_entreprise_fnc_changeVehiclesEntrepriseOwner",2];
...
if !(isNull _player) then {[player,true,_entreprise] remoteExec ["max_entreprise_fnc_updateRank",_player];};
...
hint format [(["STR_SET_ASSOCIATE"] call max_entreprise_fnc_localize),_name];
