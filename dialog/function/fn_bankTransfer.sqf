...
if ((_value + _tax) > life_fishFinder) exitWith {hint format [localize "STR_ATM_SentMoneyFail",_value,_tax]};

life_fishFinder = life_fishFinder - (_value + _tax);
...
if (LIFE_SETTINGS(getNumber,"player_moneyLog") isEqualTo 1) then {
    if (LIFE_SETTINGS(getNumber,"battlEye_friendlyLogging") isEqualTo 1) then {
        money_log = format [localize "STR_DL_ML_transferredBank_BEF",_value,_unit getVariable ["realname",name _unit],[life_fishFinder] call life_fnc_numberText,[life_houseProtect] call life_fnc_numberText];
    } else {
        money_log = format [localize "STR_DL_ML_transferredBank",profileName,(getPlayerUID player),_value,_unit getVariable ["realname",name _unit],[life_fishFinder] call life_fnc_numberText,[life_houseProtect] call life_fnc_numberText];
    };
