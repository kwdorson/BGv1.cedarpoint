...
/* Database Related Settings */
    ...
    saved_virtualItems[] = { "pickaxe","cprKit","fuelEmpty","fuelFull", "spikeStrip", "lockpick", "defuseKit","storageSmall", "handcuffs", "keys","storageBig","redgull","coffee","waterBottle","apple","peach","tbacon","donuts","rabbit","salema","ornate","mackerel","tuna","mullet","catshark","turtle_soup","hen","rooster","sheep","goat","defibrillator","toolkit", "bandages", "mushrooms", "uraniumU", "uranium", "LSD", "pdrill", "codeCracker", "morphine" }; //Array of virtual items that can be saved on your player.
	...
	vehicle_insurance_multiplier_COP = 0; //Cop Vehicle Insurance Price = Vehicle Buy Price * multiplier 
	vehicle_insurance_multiplier_MEDIC = 0; //Medic Vehicle Insurance Price = Vehicle Buy Price * multiplier 
	vehicle_insurance_multiplier_OPFOR = 0; // -- NOT IN USE -- Simply left in for east support.

/* System Settings */
    /* ATM & Federal Reserve System Configurations */
    global_ATM = true; //Allow users to access any ATM on the map (Marked & Unmarked).
    noatm_timer = 30; //Time in minutes that players cannot deposit money after selling stolen gold.
    ...
    total_maxWeight = 45; //Static variable for the maximum weight allowed without having a backpack
    ...
    restricted_uniforms[] = { };
    restricted_weapons[] = { "hlc_smg_mp5a3","hlc_pistol_P226R" };
    ...
    paycheck_civ = 2200; //Payment for civillians
    paycheck_med = 5700; //Payment for medics
    ...
    seize_exempt[] = { "Binocular", "ItemWatch", "ItemCompass", "ItemMap", "FirstAidKit", "ToolKit", "Chemlight_red", "Chemlight_yellow", "Chemlight_green", "Chemlight_blue", "optic_ACO_grn_smg" }; //Arma items that will not get seized from player inventories
    ...
    vehicleShop_rentalOnly[] = { "CUP_O_T34_TKA", "sab_UN_SUV", "sab_UN_Strider" }; //Vehicles that can only be rented and not purchased. (Last only for the session)
